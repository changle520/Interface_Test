...
def get_current_time(t=0):
    """
    四种输出格式：t=0 默认输出格式：2020-11-24 18:54:18.282000
    t=3 返回当前日期：2020-11-24
    :param t: 入参
    :return: 按格式返回当前系统时间戳
    """
    ...
    if t == 0:
        return curr_time  # 格式：2020-11-24 18:54:18.282000
    elif t == 1:
        return curr_time.strftime('%Y-%m-%d %H:%M:%S')  # 格式：2020-11-24 18:11:04
    elif t == 2:
        return curr_time.strftime('%Y%m%d-%H%M%S')  # 格式：2020-11-24-18-11-04
    elif t == 3:
        return curr_time.strftime('%Y-%m-%d')  # 格式：2020-11-24
    else:
